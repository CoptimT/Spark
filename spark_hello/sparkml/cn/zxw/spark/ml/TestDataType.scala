...
object TestDataType extends App {
  ...
  def testIndexedRowMatrix(){
    val sc = new SparkContext("local","testRowMatrix")
    val vector: Vector = Vectors.dense(Array(1.0, 2.0, 3.0))
    val indexedRow:IndexedRow = new IndexedRow(1,vector)
    val seq = Array(indexedRow)
  ...
  def testCoordinateMatrix(){
    val sc = new SparkContext("local","testRowMatrix")
    val matrixEntry:MatrixEntry = new MatrixEntry(1,1,10)
    val seq = Array(matrixEntry)
