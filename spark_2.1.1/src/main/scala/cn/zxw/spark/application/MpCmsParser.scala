...
case class MpCms (
   ...
   operator_id: Option[Long],
   operator_name:  Option[String],
   operator_object_id: Option[String],
   operation_module:  Option[String],
   operation_type: Option[String],
   operator_before: Option[String],
   operator_after: Option[String],
   channel_id: Option[String],
   audit: Option[Long],
   del_code: Option[Long]
  )
...
object MpCmsParser {
  ...
  def splitLogRaw(line: String) = {
    ...
    MpCms(
      ...
      map.get("operator_id").map(_.toLong),
      map.get("operator_name"),
      map.get("operator_object_id"),
      map.get("operation_module"),
      map.get("operation_type"),
      map.get("operator_before"),
      map.get("operator_after"),
      map.get("channel_id"),
      map.get("audit").map(_.toLong),
      map.get("del_code").map(_.toLong)
    )
  }

  def main(args: Array[String]) {
    //test
    val inputPath = "spark_2.1.1/src/main/resources/project/cms"
